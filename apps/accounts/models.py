...
class UserSettings(TimeStampedModel):
    ...
    who_can_send_me_private_msg = models.CharField(choices=PERMISSION_VIEW,
                                                   ...
                                                   db_column='WhoCanSendMePrivMessages', max_length=2)
...
class EmailSettings(TimeStampedModel):
    ...
    invites_group = models.BooleanField(default=True, db_column='InviteGroup', help_text=_('Invites me to join a group'))
    ...
    comment_recommendation = models.CharField(choices=COMMENT_NOTIFICATION,
                                              help_text=_('Comments on a recommendation that I’ve given or commented on')
    ...
    comment_activity = models.CharField(choices=COMMENT_NOTIFICATION, help_text=_('Comments on an activity or review I'
                                        ' commented on'), db_column='CommentActivity', default='B', max_length=2)
    comment_qa = models.CharField(choices=COMMENT_NOTIFICATION, help_text=_('Comments on a Goodreads Q&A that I’ve '
                                  'answered or commented on'), db_column='CommentQA', default='B', max_length=2)
    like_question = models.CharField(choices=COMMENT_NOTIFICATION, help_text=_('Likes my question or answer for '
                                     'Goodreads Q&A'), db_column='LikeQuestion', default='E', max_length=2)
    list_giveaway_book_toread = models.CharField(choices=COMMENT_NOTIFICATION, default='E', help_text=_('Lists a '
                                                 'Giveaway with a book I added as To-Read'), db_column='ListBookToRead',
                                                 max_length=2)
    list_giveaway_author_fallow = models.CharField(choices=COMMENT_NOTIFICATION, default='B', help_text=_('Lists a '
                                                   'Giveaway with a book by an author I follow'),
    ...
    comment_friendship = models.CharField(choices=COMMENT_NOTIFICATION, default='B', help_text=_('Comments on my '
                                          'friendship with another user'), db_column='CommentFriendship',
                                          max_length=2)
    post_note = models.CharField(choices=COMMENT_NOTIFICATION, default='B', help_text=_('Posts notes and highlights for'
                                 ' a book or author I have previously shelved (friend/following/author/notable)'),
    ...
    discussion_new_post = models.CharField(choices=DISCUSSION_EMAIL_FREQUENCY, default='D',
                                           ...
                                           max_length=1)
    follow_discussion = models.BooleanField(default=True, help_text=_('follow_discussion'), db_column='FollowDiscussion')
...
class GroupEmailSetting(TimeStampedModel):
    user = models.ForeignKey(UserProfile, on_delete=models.CASCADE)
...
class ReadingGroupUsers(TimeStampedModel):
    ...
    who_invites = models.ForeignKey(UserProfile, on_delete=models.CASCADE, related_name='who_invites')
    invitation_answered = models.BooleanField(default=False)


@receiver(post_save,sender=ReadingGroup)
def crate_group(sender, instance, **kwargs):
